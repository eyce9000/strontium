...
/**
 * For every 2 consecutive points in a stroke, fill in with evenly spaced points
 * until the distance in between any 2 consecutive points no longer exceeds the
 * threshold distance.
 * 
...
public class InterpolateStrokeFilter extends StrokeFilter {
	...
	/**
	 * Interpolate the given stroke such that no 2 consecutive points in the
	 * stroke has a distance greater than the pre-defined spacing.
	 */
	...
	/**
	 * Interpolate the points in the given stroke using the default spacing
	 * value.
	 */
	...
	/**
	 * Interpolate the points in the given stroke using the specified spacing
	 * value.
	 */
	public static TimedStroke interpolate(TimedStroke s, double spacing) {
		...
		result.addVertex((float) s.getX(0), (float) s.getY(0),
				s.getTimestamp(0));
		double thresh = Math.pow(spacing, 2);
		for (int i = 1; i < n; i++) {
			double pts[][] = half(s.getX(i - 1), s.getY(i - 1),
					s.getTimestamp(i - 1), s.getX(i), s.getY(i),
					s.getTimestamp(i), thresh);
			if (pts != null) {
				for (int j = 0; j < pts.length; j++) {
					result.addVertex((float) pts[j][0], (float) pts[j][1],
							(long) pts[j][2]);
				}
			}
			result.addVertex((float) s.getX(i), (float) s.getY(i),
					s.getTimestamp(i));
		}
		return result;
	}
	 ...
	 */
	private static double[][] half(double x1, double y1, double t1, double x2,
			double y2, double t2, double thresh) {
